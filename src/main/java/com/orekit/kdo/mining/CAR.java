...
public class CAR {
     ...
     */
    public CAR(int numberOfObservations, Collection<DrivingFeature> drivingFeatures) {
     ...
     */
    public CAR(int[][] dataset, boolean negate) {
        ...
        for (int attr = 0; attr < dataset[0].length; attr++) {
            HashMap<Integer, BitSet> featMatches = new HashMap<>();
            //loop over the observations
            for (int obs = 0; obs < dataset.length; obs++) {
                int level = dataset[obs][attr];
            ...
            //Create the features for this attribtue
            for (Integer level : featMatches.keySet()) {
                this.baseFeatures.add(
                        new DrivingFeature(
                                String.format("%d = %d", attr, level),
